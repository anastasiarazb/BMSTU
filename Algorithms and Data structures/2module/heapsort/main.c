...
#define MAX_LINES 100 //Максимальная длина вводимой строки, включая '\0' и счетчик букв а.
                      //Т.к. длина строки < 100, то счетчик по а < 256 => тостаточно 1 char
...
int main(void)
{
    size_t number;                              //Количество строк
    scanf("%zd", &number);
             ...
             int (*compare)(const void *, const void *))
{
    ...
    while(1){
//        printf("i %s left %s right %s\n", i/width, left/width, right/width);
