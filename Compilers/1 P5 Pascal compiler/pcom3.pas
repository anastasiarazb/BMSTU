...
(*-------------------------------------------------------------------------*)
  ...
  begin
    if (c >= 'A') & (c<= 'Z') then c := chr(ord(c)-ord('A')+ord('a'));
    lcase := c
  end { lcase };
  ...
  begin l := maxids; p := nil; a := nil; j := 1;
    while (l > 1) & (b[l] = ' ') do l := l-1; { find length of fixed string }
    if b[l] = ' ' then l := 0;
  ...
  begin ca := ' '; cb := ' ';
    while (a <> nil) | (b <> nil) do begin
      i := 1;
      while (i <= varsqt) and ((a <> nil) | (b <> nil)) do begin
...
1:  fcp := lcp
  ...
  procedure inittables;
    ...
    begin
      ...
      rsy[ 4] := tosy;      rsy[ 5] := relop;     rsy[ 6] := addop;   (*rw[ 6] := 'or       ';*)
      rsy[ 7] := endsy;     rsy[ 8] := forsy;     rsy[ 9] := varsy;
      rsy[10] := mulop;     rsy[11] := mulop;     rsy[12] := setsy;
      rsy[13] := mulop;     rsy[14] := notsy;     rsy[15] := nilsy;   (*rw[13] := 'and      '; rw[14] := 'not*)
      ...
      ssy['&'] := mulop;    ssy['~'] := notsy;    ssy['|'] := addop; (*like synonimes in rw table*)
...
begin
  ...
  writeln(prr, 'i Pascal intermediate file Generated by P5 Pascal compiler vs. ', 
          majorver:1, '.', minorver:1, ' modified by Nastasia, part2, complete.');
  writeln(prr, 'i');
  insymbol;
